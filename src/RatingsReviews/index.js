...
export default function RatingsReviews(props) {
  if (props.dev.logs) {
    mainRenderCount++;
    props.dev.renders &&
      console.log('DEV  RENDER   RelatedProducts     number of renders: ', mainRenderCount);
    props.dev.state && console.log('DEV  STATE   RelatedProducts: ', props.reviewData);
  }

  const [fullSummary] = React.useState(false);
  ...
  function SortReviews() {
    ...
    return (
      <>
        <div>
          {props.reviewData.results.length} reviews
          <select
            onChange={(e) => {
  ...
  function NormalReviewDisplay() {
    return sortedReviews.slice(0, diplayedReviewCount).map((review, id) => {
      return (
        <div key={id}>
  ...
  function FilteredReviewDisplay() {
    ...
    return filter(reviews, keyword)
      .slice(0, diplayedReviewCount)
      .map((review, id) => {
        return (
          <div key={id}>
            <div>{moment(review.date).format('MMM Do YY')}</div>
  ...
  return <div data-testid='reviews'>Loading...</div>;
}
