...
const RelatedProducts = () => {
  ...
  const relatedInit = related.length
    ? Array(related.length).fill({})
    : [{ type: 'emptyRelated', id: state.currentProduct }];
  const [relatedItemData, setRelatedItemData] = useState(relatedInit);
  const outfitInit = outfit.length
    ? Array(outfit.length).fill({})
    : [{ type: 'emptyOutfit', id: state.currentProduct }];
